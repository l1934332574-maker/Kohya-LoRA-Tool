...
**当前版本：v0.15.0** · [GitHub Releases](https://github.com/l1934332574-maker/Kohya-LoRA-Tool/releases) · [国内安装包（魔搭）](https://modelscope.cn/models/FGtiancai/Kohya-LoRA-Tool)
...
- **新增标签管理中英词典（v1 起步）**：标签编辑器内置离线中英词典（17.5 万+ 词条，随包发布、无需联网）；输入英文自动补全并显示中文（数据集内出现过的标签优先）、输入中文反查英文标签、一键翻译整张图标签，双击/点「插入」即可加入当前图片；「标签统计」每行直接显示中文翻译。
- **修复 WD14 自动打标“没打上”（标签只剩 1girl, solo）**：打标前自动补装 cv2/imagesize 依赖；官方打标脚本缺失或环境损坏时，自动改用内置 WD14 打标（onnxruntime 直读随包模型，不依赖第一引擎）；上次失败留下的兜底标签会被自动清掉并重新打标，第四引擎 / AMD 用户同样适用。
...
- 各引擎的 PyTorch 大轮子统一使用阿里云/上海交大国内镜像断点续传，避免无代理用户被迫访问海外源。
...
| 文件 | 说明 |
|---|---|
| `Setup.exe` | 推荐使用。双击安装，内置主程序、离线安装资源、WD14 打标模型和离线中英词典；当前最新版为 v0.15.0 |
