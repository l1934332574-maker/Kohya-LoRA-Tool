...
- **训练分辨率可调**：高级参数面板可改训练分辨率（512/768/1024），16G 显存跑 Krea 2 / SDXL 可降到 768/512 防爆显存
- **🎬 视频 LoRA（MiniMax H3，实验性）**：第三训练引擎 AI Toolkit，33.1B 全模态视频模型 T2V LoRA；视频数据集（mp4+字幕）自动抽帧；**应用内模型下载器**（断点续传）+ **Qwen2.5-VL 视频自动打标**；推荐 24G NVIDIA 显存
- **应用内下载全覆盖**：SD1.5 / SDXL / Anima 底模 + FLUX 四件套 + Krea2 + H3 全部支持应用内下载（国内镜像、断点续传、下完自动识别）
- **🖼 Qwen-Image / Z-Image LoRA（实验性）**：AI Toolkit 引擎，支持 Qwen-Image-2512（20B，16G 起步/24G 舒服）与 Z-Image（8B，12G 起步/16G 舒服）；首次训练自动下载模型（国内镜像）
- **智能新手引导**：左侧引导按所选模式动态生成（只显示该模式需要的步骤），主页不显示；每步完成自动高亮下一步，全部完成才点亮一键训练
