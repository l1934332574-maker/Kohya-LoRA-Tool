...
## 🛡️ 安装被 Windows / 杀毒软件拦截？

本项目**没有代码签名证书**（OV/EV 证书年费数千元），因此 Windows 和部分杀毒软件会把安装包提示为风险 —— **这是误报，安装包不含病毒**。

按你看到的现象对应处理：

| 现象 | 怎么处理 |
|---|---|
| 蓝框「**Windows 已保护你的电脑**」 | 点框里的 **「更多信息」** → 出现 **「仍要运行」** → 点它 |
| 安全中心通知「**检测到威胁**」 | Windows 安全中心 → 病毒和威胁防护 → **保护历史记录** → 找到该项 → **允许在设备上** |
| 反复被拦 | 病毒和威胁防护 → 管理设置 → **排除项** → 添加排除项 → 文件夹 → 选安装目录 |
| 360 / 火绒 / 电脑管家 | 在「信任区 / 白名单」里加入安装包或安装目录 |
| 右键属性里有「解除锁定」 | 勾选 **解除锁定** → 确定，再双击 |

**想自己核验文件没被篡改**：安装包 `Setup.exe` 的 SHA256 会随每个版本公布在 Release 说明和 `update.json` 的 `setup_sha256` 字段里。可用 PowerShell 比对：

```powershell
Get-FileHash .\Setup.exe -Algorithm SHA256
```

软件内「🔄 检查更新」下载完成后也会自动做这个校验，校验不通过会拒绝安装（不会拿坏包覆盖你现有的可用版本）。
