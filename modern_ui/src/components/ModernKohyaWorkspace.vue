...
function previewOnly(action: string) {
  emit('notify', props.desktop ? `${action}暂未接入新版训练页。` : `${action}仅展示界面效果，当前不会调用本机训练功能。`)
}
...
<template>
  <section class="kohya-workspace" aria-label="Kohya LoRA 新版训练页">
...
.kohya-heading { gap:16px; min-width:0; }.kohya-heading-copy { gap:12px; min-width:0; }.kohya-heading-copy h1 { margin:0; color:var(--tone-d1d4da); font-size:17px; font-weight:350; white-space:nowrap; }.kohya-heading-copy span { overflow:hidden; color:var(--hint); font-size:11px; text-overflow:ellipsis; white-space:nowrap; }
.kohya-back,.kohya-button { display:inline-flex; align-items:center; justify-content:center; gap:6px; min-height:31px; padding:0 10px; border:1px solid var(--border); border-radius:5px; color:var(--tone-afb5bf); background:transparent; font-size:11px; cursor:pointer; transition:background-color 140ms ease,border-color 140ms ease,color 140ms ease,transform 110ms ease-out; }.kohya-back:hover,.kohya-button:hover { border-color:var(--tone-4c525d); color:var(--tone-d2d5db); background:var(--tone-2a2e36); }.kohya-back:active,.kohya-button:active { transform:translateY(1px); }.kohya-back .ui-icon { width:13px; height:13px; }.kohya-toolbar-actions { gap:8px; }.kohya-button.primary { border-color:transparent; color:var(--tone-e5e7eb); background:var(--tone-626f81); }.kohya-button.primary:hover { background:var(--tone-6b7788); }.kohya-button.primary .ui-icon { width:13px; height:13px; }.kohya-button.compact { min-height:28px; padding:0 9px; font-size:10px; white-space:nowrap; }
.kohya-status-line { display:flex; flex:0 0 auto; gap:7px; min-height:35px; align-items:center; padding:3px 24px 6px; }.status-sample { display:inline-flex; align-items:center; height:23px; padding:0 9px; border:1px solid var(--tone-3b3e46); border-radius:4px; color:var(--tone-9da4ae); background:var(--tone-262930); font-size:10px; }
.kohya-scroll-area { flex:1 1 auto; min-height:0; overflow:auto; scrollbar-color:var(--tone-4a4e56) transparent; scrollbar-width:thin; }.kohya-scroll-area::-webkit-scrollbar { width:9px; }.kohya-scroll-area::-webkit-scrollbar-thumb { border:2px solid var(--bg); border-radius:8px; background:var(--tone-4b5059); }.kohya-scroll-content { display:flex; flex-direction:column; gap:11px; padding:5px 26px 13px 24px; }
.base-summary { display:flex; align-items:center; gap:10px; min-height:58px; padding:8px 11px; border:1px solid var(--outline-subtle); border-radius:7px; background:var(--card); }.base-mark { display:grid; width:32px; height:32px; flex:0 0 auto; place-items:center; border:1px solid var(--tone-414752); border-radius:6px; color:var(--tone-aeb6c1); background:var(--tone-2b2f38); }.base-mark .ui-icon { width:16px; height:16px; }.base-copy { display:grid; flex:1; min-width:0; gap:2px; }.base-copy span,.output-name > span { color:var(--hint); font-size:9px; }.base-copy strong { overflow:hidden; color:var(--tone-cdd1d8); font-size:12px; font-weight:400; text-overflow:ellipsis; white-space:nowrap; }.base-copy small { overflow:hidden; color:var(--tone-858b95); font-size:9px; text-overflow:ellipsis; white-space:nowrap; }.base-type-select { width:min(280px,100%); height:26px; min-height:26px; padding-left:6px; font-size:10px; }.output-name { display:grid; width:min(220px,24%); gap:4px; }.output-name .kohya-input { height:27px; }
.kohya-columns { display:grid; grid-template-columns:minmax(0,1fr) minmax(0,1fr); align-items:start; gap:11px; }.kohya-column { display:flex; min-width:0; flex-direction:column; gap:11px; }.kohya-card { min-width:0; padding:12px 13px 11px; border:1px solid var(--outline-subtle); border-radius:7px; background:var(--card); transition:background-color 150ms ease,border-color 150ms ease; }.kohya-card:hover { border-color:var(--outline-hover); background:var(--tone-292c34); }
.card-heading { display:flex; align-items:center; gap:9px; min-height:25px; margin-bottom:10px; }.step-number { color:var(--tone-8993a0); font-size:10px; letter-spacing:.04em; }.card-heading > div:not(.parameter-title) { display:grid; gap:2px; }.card-heading h2 { margin:0; color:var(--tone-cbd0d7); font-size:13px; font-weight:400; }.card-heading small { color:var(--hint); font-size:9px; }.parameter-heading { margin-bottom:10px; }.parameter-title { display:flex; flex:1; align-items:baseline; gap:8px; }
.folder-row { display:flex; align-items:center; gap:9px; min-height:43px; padding:6px 8px; border:1px solid var(--tone-393d46); border-radius:5px; background:var(--tone-22252c); }.folder-row > .ui-icon { width:15px; height:15px; color:var(--tone-9da7b5); }.folder-copy { display:grid; flex:1; min-width:0; gap:3px; }.folder-copy span,.kohya-field > span { color:var(--hint); font-size:10px; }.folder-copy strong { overflow:hidden; color:var(--tone-bfc4cc); font-size:11px; font-weight:350; text-overflow:ellipsis; white-space:nowrap; }.dataset-counts { display:flex; align-items:center; gap:9px; min-height:28px; color:var(--tone-8f959f); font-size:10px; }.dataset-counts b { color:var(--tone-c2c7cf); font-weight:400; }.dataset-counts > i { width:1px; height:11px; background:var(--tone-454951); }.sample-state { margin-left:auto; color:var(--tone-9298a2); }.field-row { display:flex; align-items:end; gap:10px; }.field-row.equal-columns > * { flex:1; }.kohya-field { display:grid; min-width:0; gap:5px; }.kohya-input,.kohya-select { width:100%; min-width:0; height:30px; padding:0 9px; border:1px solid var(--tone-3d414a); border-radius:5px; color:var(--tone-c6cbd3); background:var(--tone-22252c); font-size:11px; font-weight:350; }.kohya-select { padding-right:22px; cursor:pointer; }.kohya-select option { color:var(--tone-cbd0d7); background:var(--tone-252830); }.kohya-input::placeholder { color:var(--tone-707680); }.card-hint,.inline-hint { margin:8px 0 0; color:var(--hint); font-size:10px; line-height:1.4; }.inline-hint { align-self:center; }.spaced-row { margin-top:9px; }
...
.check-line { display:inline-flex; align-items:center; gap:7px; min-height:28px; padding:2px 3px; border:0; color:var(--tone-a7adb7); background:transparent; font-size:10px; text-align:left; cursor:pointer; }.check-line > i { display:grid; width:14px; height:14px; flex:0 0 auto; place-items:center; border:1px solid var(--tone-646a75); border-radius:3px; background:var(--tone-22252c); transition:border-color 130ms ease,background-color 130ms ease; }.check-line.checked > i { border-color:var(--tone-687689); background:var(--tone-626f81); }.check-line.checked > i::after { width:6px; height:3px; border-bottom:1.4px solid var(--tone-e2e5e9); border-left:1.4px solid var(--tone-e2e5e9); transform:translateY(-1px) rotate(-45deg); content:''; }.check-line:hover { color:var(--tone-d0d4da); }.switch-line { display:flex; align-items:center; gap:8px; width:100%; margin-top:10px; padding:5px 0 0; border:0; color:inherit; background:transparent; text-align:left; cursor:pointer; }.switch-track { position:relative; width:27px; height:15px; flex:0 0 auto; border-radius:10px; background:var(--tone-454951); transition:background-color 160ms ease; }.switch-track i { position:absolute; top:2px; left:2px; width:11px; height:11px; border-radius:50%; background:var(--tone-aeb3bb); transition:transform 170ms var(--ease-out),background-color 160ms ease; }.switch-line.enabled .switch-track { background:var(--tone-626f81); }.switch-line.enabled .switch-track i { background:var(--tone-e0e3e8); transform:translateX(12px); }.switch-line > span:last-child { display:grid; gap:2px; }.switch-line strong { color:var(--tone-bfc4cc); font-size:11px; font-weight:400; }.switch-line small { color:var(--hint); font-size:9px; }
.parameter-grid { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:8px; }.concept-row .check-line { align-self:end; }.advanced-grid { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); align-items:end; gap:9px; margin-top:10px; padding:10px; border:1px solid var(--tone-363a43); border-radius:5px; background:var(--tone-23262d); }.advanced-grid .kohya-select,.advanced-grid .kohya-input { height:29px; }.chevron { display:inline-block; margin-left:5px; transition:transform 180ms var(--ease-out); }.chevron.open { transform:rotate(180deg); }.kohya-accordion-enter-active,.kohya-accordion-leave-active { transition:max-height 190ms var(--ease-out),opacity 140ms ease,transform 180ms var(--ease-out); }.kohya-accordion-enter-from,.kohya-accordion-leave-to { max-height:0; opacity:0; transform:translateY(-3px); }.kohya-accordion-enter-to,.kohya-accordion-leave-from { max-height:100px; opacity:1; transform:translateY(0); }
.parameter-scope-note { grid-column:1/-1; margin:0; color:var(--hint); font-size:10px; line-height:1.4; }
.utility-row { display:flex; flex-wrap:wrap; gap:7px; padding:2px 1px; }.utility-button { min-height:29px; padding:0 10px; border:1px solid var(--border); border-radius:5px; color:var(--tone-afb5bf); background:var(--tone-252830); font-size:10px; cursor:pointer; transition:border-color 130ms ease,color 130ms ease,background-color 130ms ease,transform 110ms ease-out; }.utility-button:hover { border-color:var(--tone-505660); color:var(--tone-d0d4da); background:var(--tone-2b2e36); }.utility-button:active { transform:scale(.985); }.preview-note { display:flex; align-items:center; gap:7px; margin:-3px 1px 0; color:var(--tone-818792); font-size:10px; }.preview-note > span { width:5px; height:5px; border-radius:50%; background:var(--tone-929baa); }
