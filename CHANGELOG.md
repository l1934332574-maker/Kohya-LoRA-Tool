...
- 修复预处理复用旧 venv 时的 Python 基座错配：按 pyvenv.cfg 记录版本匹配基座，并校验配置与实际运行时，避免混版本环境导致依赖导入失败。
- 外观设置新增背景图片历史，可调整背景图片和界面组件的透明度，并可开启界面闲置渐隐。
...
- 可手动指定已有的文本编码器和 VAE safetensors；留空时自动查找 ComfyUI 模型目录。
- 缺少 processor/分词器和模型配置时，首次只下载约 16 MB 小文件，不会重下已有的大权重。
- 保存、采样间隔会按当前引擎显示「步」或「轮」；切换模式时分别保留对应单位的设置。

### 验证

- Qwen-Image-2.1 本地组件路径、配置与 processor 缓存、引擎更新及回归测试通过。
- 完整 GPU 训练未在本机执行；请用户使用本机实际模型和显卡验证训练链路。
...
- 模型选择窗口新增单独的 safetensors 文件入口，并只在选择 Qwen-Image-2.1 时启用。
- 识别本地 `qwen_image_2.1_*.safetensors` 权重，训练配置直接把文件写入 `name_or_path`；完整 Diffusers 目录仍按原方式校验。
- 单文件不包含配置、文本编码器和 VAE；首次训练需联网从 Qwen 官方仓库读取这些组件。

### 验证

- 单文件选择、架构限制、训练前就绪检查和 YAML 路径回归测试通过。
- 当前环境未执行完整 Qwen-Image 实际训练。

---

## v0.17.15（2026-09-23）

### 新增：AI Toolkit 引擎更新提示和一键更新

- 第三引擎已安装、但缺少 Qwen-Image-2.1 所需架构时，顶部显示动态箭头更新提示。
- 点击提示可查看更新说明并一键更新 AI Toolkit 源码。
- 更新前备份原源码，更新后检查架构注册；保留 Python 环境和已下载模型，失败时恢复旧源码。

### 优化：Qwen-Image 模型选择

- 可直接从下拉框选择 Qwen-Image-2512 或 Qwen-Image-2.1，也可指定已有的本地模型目录。
- 训练确认窗口显示本次实际使用的模型仓库。

### 验证

- 引擎更新成功、失败回滚和界面事件链路的离线回归测试通过。
- 当前环境未执行完整 Qwen-Image 实际训练。
...
- **用户反馈**（AMD RX 7800 XT / 15.8G，日志 KohyaLoRA_055_20260922）：
  「0.17.10 还是好好用的，一更新就出问题了」→ 核对后**属实** ✗ —— 是**我们误伤** ✗
- **现象**：升到 v0.17.11 后弹「AMD 依赖已安装，但环境验证失败」✗ → **无法继续** ✗
- **根因**：v0.17.11 新加的深度自检里，把 `torch._C._distributed_c10d` 当成了**必需项** ✗
  - 日志里他报的是 `'torch._C' is not a package` ✗ —— 这恰恰说明 **`import torch` 是成功的** ✓
    （若 torch 本身 import 失败，根本走不到"解析 `torch._C`"这一步 ✓）
  - → **AMD ROCm 版 PyTorch 可能本来就不含** `torch/_C/_distributed_c10d` ✓
    （它只影响 distributed / DTensor 路径 ✓ 单卡训练未必走到 ✓）
  - → 于是**能用的环境被判定成坏的** ✗，还被"拦下训练"✗
- **修复**：
  · 该子模块（含 `torch.distributed.tensor`）**降为「仅警告」** ✓
    必需项收敛为 `torch` / `torchvision` / `transformers` 三个 ✓（它们失败才是真的必然崩 ✓）
  · `verify_amd_torch` 里把子模块检查**单独 try** ✓ —— 不再拖垮主判定 ✓
    （实测：子模块缺失时退出码为 **0** ✓，只打印 `SUBMODULES_OK=0` ✓）
  · 措辞纠正 ✗：不再说「PyTorch 装得不完整」➜ 改为「**不等于环境坏了**：
    AMD ROCm 版可能本来就不含；单卡可先照常训练 ✓，若训练**中途**报此错再重装」✓
  · `env_selftest_advice` 现在**区分必需 / 非必需** ✓ —— 只有必需项失败才说
    「训练没有开始、没白跑时间」✓；仅非必需失败则明确写「**不影响开始训练**」✓
- **澄清**：这**不是**说日志 ③（AMD 9060 XT）那次是误报 ✓ —— 他确实是**训练中途崩了** ✓
  只是**不能靠"缺这个模块"就断定"包装坏了"** ✗（这两件事必须分开 ✓）

### 新增测试（防止再次误伤）

- `ENV_SELFTEST_CATCHES_BROKEN_ENV_OK` 补了**反向断言** ✓：
  · `torch._C._distributed_c10d` **不许**被标成「必需」✓
  · torch 本体可用、只是缺分布式扩展时 → **自检必须通过（不许拦训练）** ✓
  · 仅非必需项失败时，文案必须含「不影响开始训练」，且**不许**出现「没白跑训练时间」✓
  · 必需项真失败时，**必须**出现「没白跑训练时间」✓

### 验证

- 冒烟 **✔ 全部通过** ✓ 引擎套件 **ALL_ENGINE_CONTROL_FLOW_TESTS_OK** ✓ 静态检查 0 错误 ✓
- **真 venv 实测**：造「有 torch、没有 `_C`」的环境 ✓ → 子模块那段退出码 **0** ✓
  （v0.17.11 是 1 ✗ —— 就是它把用户卡住的 ✓）

---

## v0.17.11（2026-09-22）

### 修复★：环境「装坏了」却拖到训练一半才炸 —— 新增深度环境自检

- **背景**：同一天收到**三份**用户日志，**三台完全不同的机器**，**三种坏法、一个都没提前发现** ✗
  · ① RTX 5070 12G：`torch/lib/c10.dll` 初始化失败（VC++ 运行库 14.36 < PyTorch 需要的 14.44）
  · ② RTX 5060 Ti 16G：`transformers/models` 目录**损坏**（WinError 1392）
  · ③ AMD RX 9060 XT 8G：`torch/_C/` 目录**缺失** → 报 `'torch._C' is not a package`
- **根因**：环境检查只用 `find_spec()`（**只看文件/目录在不在** ✗），或只 `import torch`
  —— 而 `import torch` **根本用不到** `_C._distributed_c10d` ✗
  → 三种坏法**全部通过** ✗ → 全都拖到「训练跑到一半」才炸 ✗
  （①②③ 的数据检查、latents 缓存、文本编码器缓存、模型加载**全白跑** ✗）
- **做法**：新增**深度环境自检** —— 真的把模块 import 一遍（含子模块 ✓）
  · 依赖装好后**立刻**自检（第一引擎 / 第四引擎 / AMD 三条路都接上 ✓）
  · 坏了的**按人话翻出修法** ✓，而不是「退出码 1，请查看上方日志」
  · **训练不开始** ✓ → 不白跑 ✓、项目配置不用重设 ✓
...
| 现象 | 给出的修法 |
|---|---|
| `1392` 目录损坏 | 先看磁盘空间 → 重跑【② 安装训练内核】重建；仍报 1392 → 可能坏道，把工具换盘重装 |
| `torch._C` 不完整 | 重跑【② 安装训练内核】；AMD 环境重跑 AMD 引导（或删 `venv_amd`/`fizgig_venv` 再装） |
| `1114` DLL 初始化失败 | 装最新 VC++ 运行库（附直链）+ **必须重启** + 怎么确认真的装上了 |

- **AMD 那条路**：`verify_amd_torch` 原来只 `import torch` ✗
  → 现在连带 `torch._C._distributed_c10d`、`torch.distributed.tensor` 一起验 ✓
  （日志 ③ 那种坏法，从此会在**安装阶段**就暴露 ✓ 而不是等训练 ✗）

### 修复★：16G 卡上 **NF4 也被无故降到 512px**（白丢画质）

- **现象**：Krea2(Fizgig) 的分辨率钳制**不看量化方式** ✗ —— 凡是 ≤19G 一律砍到 512 ✗
  → **连选 NF4 的用户也被砍** ✗
- **可 NF4 底模只占 ~5.6GB** ✓（工具自己的提示里就写着 ✓）
  → 16G 卡上 **NF4 + 768px 完全放得下** ✓ → 砍到 512 纯属**白丢画质** ✗
- **真正需要降的是 int8**：它的底模必须**全驻留**（引擎强制 `blocks_to_swap=0` ✗），
  需 ~18G > 16G ✗ → 溢出换页 → 越跑越慢（实测 8~19 s/it ✓）→ 这条保护**保留** ✓
- **修复**：按**最终量化档**判断 ✓ —— 只有 int8 才降 ✓；
  NF4 / fp8 / ≥20G / 本就 ≤512 **一律原样** ✓
  （判 `quant_flags` 而不是用户填的「自动」✗：auto 在 <10G 会变成 NF4、12~19G 会变成 int8 ✓）

### 新增测试

- `ENV_SELFTEST_CATCHES_BROKEN_ENV_OK` —— **真建一个 venv、真塞"有 torch 但没 `_C`"的假包** ✓
  断言：必须点名 `torch._C._distributed_c10d` ✓ / 必须标「必需」（否则等于不拦 ✗）/
  `torch` 本体**不许误报** ✓ / 三份日志的三种坏法文案都能识别 ✓
- `FIZGIG_CLAMP_ONLY_INT8_OK` —— int8 降 512 ✓；**NF4 / fp8 / 24G / 显存未知一律不动** ✓
- 同步修正两处既有测试：`test_fizgig_int8_vram_guard`（原调用是旧签名 ✗）、
  `test_fizgig_deps_self_heal`（补上「包在但坏了 → 必须返回 False 拦下训练」✓）

### 验证

- 冒烟 **✔ 全部通过** ✓ 引擎套件 **ALL_ENGINE_CONTROL_FLOW_TESTS_OK** ✓ 静态检查 0 错误 ✓
- 深度自检在**真 venv** 上实测：空环境报 8 项 ✓、假 torch **精准点名 `_C` 且不误报** ✓
- 两段**内嵌在字符串里的代码**（AMD 探测器 / 自检注入）做了编译 + 实跑校验 ✓

---

## v0.17.10（2026-09-21）

### 修复★★：「模型保存间隔」单位错了 —— 填 200 就是"每 200 轮"，**一个快照都不会存**

- **用户反馈**（欣欣 / Krea2 Fizgig，界面截图佐证）：
  「输出的 LoRA 快照好像也没了，以前都有很多个，新版就没了」
- **根因**：Krea2/FLUX.2（含 Fizgig）这条路的保存间隔单位是**轮** ✗
  （`--save_every_n_epochs <save_every>`，见四处训练入口）
  他界面里填的是 **200** ✗ → 200 轮存一次，而训练只有 18~22 轮 ✗
  → **一个中间快照都不会产生** ✗ 日志只留一句「本次没有产生可续训的快照」✗
  → 他读成"新版不存快照了"✗
- **他为什么会填 200** ✗：原界面提示把「留空=默认 200 **步**」和「N **轮**」写在**同一句**里 ✗
  （与上一版的「采样预览间隔」是**同一类单位混淆** ✓）
- **修复**：
  1. 界面标签按模式动态 ✓：Krea2/FLUX.2（含 Fizgig）显示「模型保存间隔**(轮)**」，
     画风/人物/视频/Qwen/Z-Image 显示「**(步)**」✓
  2. 训练开始时**打印实际含义** ✓：`模型保存间隔：每 1 轮存一次（共 18 轮）`
  3. ★ **值超过总轮数时明确警告** ✓：
     ```
     ⚠ 模型保存间隔 = 200 轮，超过总轮数 18 → 本次不会保存任何中间快照 ✗
       后果：中断后无法续训，也拿不到中间轮次的 LoRA 权重 ✗
       改法：把它改小（如 1~2 轮），或留空（默认每 1 轮存一次）✓
     ```
     （不再只是让用户自己从"没有产生快照"里猜 ✓）
- 四处训练入口（Krea2 / FLUX.2 / Krea2(Fizgig) / FLUX.2(Fizgig)）统一接入 ✓

### 改进★：16G 档跑 Krea2(Fizgig) 自动把分辨率降到 512

- 与同门 **Krea2(AI-Toolkit)** 对齐 ✓（那条路早就有 `_is_low → min(reso, 512)` ✗ 而 Fizgig 一直漏着 ✗）
- **明确打印**，不静默改用户的分辨率 ✗（这是反复踩到的教训 ✓）
- ⚠️ 文案里如实写明：**512 只是缓解，不是根治** ✗

### 新增★：int8「全驻留」告警（块交换被引擎忽略）

- **实测证据**（用户日志，5060 Ti 16G）：
  ```
  [int8] W8A8 base is fully resident (staged quantise -> GPU)
  — block swap can't reduce its footprint; forcing blocks_to_swap=0.    ← 引擎强制改 0 ✗
  ```
  → **工具按显存配的 blocks_to_swap 对 int8 完全无效** ✗（传 20 也被改成 0 ✗）
  → 显存压力全落在「底模常驻 + 激活」上 → 装不下就换页 → **时快时慢**（3~11 秒/步）✗
- 现在只要这句出现就**当场告警** ✓，并说清：
  · 工具传的块交换**已被引擎忽略** ✓
  · 可选：改 **NF4 4bit** / 换 24G+ 卡 / 降 512 只是缓解 ✗
- Krea2(Fizgig) 与 FLUX.2(Fizgig) 两条路都接上 ✓

### 澄清（写进日志，避免继续误判）

**Fizgig 与 musubi 是两套引擎，速度不能直接对比** ✓
- `FIZGIG_VERSION = "v5.0.0"` 自 v0.14.0 起**钉死未变** ✓ → 不是引擎升级引起的 ✗
- 用户提供的"旧版 2 秒/步"截图经比对：进度条是 `lr: … loss: …`（**kohya/sd-scripts 风格** ✗），
  而 Fizgig 是 `avr_loss=` ✗；且它**第 16 步就 2 秒** ✗ —— 而 Fizgig **前 2 个 epoch 是预热**
  （引擎原文），第 16 步不可能这么快 ✗
  → 那张图**极可能是 Krea2（musubi 引擎）** ✓（其基准正是 int8 + 512px ≈ **2.5 s/步** ✓）
  → 想要那种速度：**切回 Krea2（musubi 引擎）** ✓ 而不是"修 Fizgig" ✓

**验证**（真机）：

| 场景 | 结果 |
|---|---|
| 保存间隔 200 / 只有 18 轮 | **警告"不会保存任何快照"** ✓ + 给改法 ✓ |
| 填 1 / 2 / 留空 / 正好等于轮数 | 正常提示、**不误报** ✓ |
| 界面标签（6 个模式） | krea2 / krea2_fz / flux2 / flux2_fz → **(轮)** ✓；character / video → **(步)** ✓ |
| int8 全驻留（用用户日志逐字原文） | **命中并告警** ✓；普通/空日志 → 不误报 ✓ |

- 新增回归测试 `SAVE_INTERVAL_IS_EPOCHS_OK`、`FIZGIG_INT8_VRAM_GUARD_OK` ✓

---

## v0.17.9（2026-09-21）

### 修复★★：「采样预览间隔」在 Fizgig 引擎下**单位错了** —— 填 10 得到的是"每 1 轮"，差 10 倍

- **用户实测反馈**（原文）：「还是 100 张预览一次，而且预览过会变慢」
  追问后他点明关键：「**那不是标的轮吗？所以填的 10 轮一次啊**」✓ 他说得对 ✗
- **根因**（设计问题，不是用户填错 ✓）：
  | | 界面标的 | 代码实际做的 | 他填 10 得到 |
  |---|---|---|---|
  | v0.17.4 起 | 「采样预览间隔**(步)**」✗ | 当**步**数再换算 `round(10÷100)=0` → 下限 1 ✗ | **每 1 轮** ✗ |
  | 他理解的 | 「**(轮)**」✓ | 直接就是轮数 | 应为**每 10 轮** ✓ |
  → **差 10 倍** ✓ 与他看到的「还是 100 张预览一次」完全吻合（1 轮 = 100 步 ✓）
  → 且提示写的是「换算成 round(N÷每轮步数) 轮」✗ —— 用户看不懂，等于没提示 ✗
- **修复**：Fizgig 下这个字段**直接就是「轮」** ✓
  · 填 10 → 每 **10 轮**（约每 1000 步）✓
  · 留空 / 0 → 沿用「约每 100 步」启发式（= 每 1 轮）✓
  · 填超总轮数 → 限制在总轮数，并**提醒**「本次只会出 1 张预览」✓
  · **界面标签与提示随模式变** ✓：Fizgig 显示「采样预览间隔**(轮)**」，
    其它模式仍为「**(步)**」—— 不再看错单位 ✓
    （提示原文：「本引擎按「轮」出图 —— 直接填轮数：填 10 = 每 10 轮一次；留空=每 1 轮。
      它做不到「每 N 步」✗」）
  · 训练日志打印实际值：`采样预览：每 10 轮（约每 1000 步）…` ✓

### 修复★：「预览过会变慢」—— **是误解**，但工具以前没解释清楚

- **用户日志原始数据**（同一份日志内）：
  ```
  epoch 2 末  200/4000 [11:22, 3.41s/it]
  epoch 3 首  201/4000 [13:19]        ← 200→201 之间 117 秒 = 预览本身
  之后逐步    13:19 → 13:22 → 13:25   ← 稳定 3 秒/步 ✓ 与 epoch 2 完全一致
  ```
  → **实际训练步速一点没变** ✓ 只是**界面那个累计 s/it** 把预览的 ~2 分钟算进去了 ✗
- **修复**：训练开始时打印一句说明 ✓
  「每次预览本身约 1~2 分钟，它会**计入界面的累计 s/it**，所以每次预览后平均值会变大
   —— 这是正常的，训练步速并未变慢 ✓（判断真实速度看引擎自报的「epoch N avr_loss=…」那一行）」

### 说明：Fizgig 引擎**做不到**「每 N 步」（引擎限制，非本工具）

- Fizgig 按**轮**组织采样（epoch-0 的 Sample at Start + 每个 epoch 各一次，
  见源码核对结论 `_warn_fizgig_sample_failure` 的 docstring）✓
- 需要「每 N 步」→ 用 **Krea2（musubi 引擎）** ✓ 它支持 `--sample_every_n_steps` ✓
  （代价：需要安装第二引擎，日志显示该用户第二引擎未安装 ✗ 所以本次先在界面说清限制 ✓）

**真机验证**（每轮 100 步 / 共 40 轮）：

| 输入 | 结果 |
|---|---|
| 填 10 | 每 **10 轮** ✓（他想要的就是这个） |
| 填 1 / 3 | 每 1 轮 / 每 3 轮 ✓ |
| 留空 / 0 | 每 1 轮（约每 100 步）✓ |
| 填 100（>40 轮） | 限制为 40，并提醒只会出 1 张 ✓ |

- 界面标签实测：`krea2_fz / flux2_fz` → 「(轮)」✓；`krea2 / character` → 「(步)」✓
- 新增回归测试 `FIZGIG_SAMPLE_INTERVAL_IS_EPOCHS_OK` ✓
  （含「留空仍约每 100 步」「超总轮数要夹住」「标签必须随模式切」等断言）

> ⚠️ 预览频率是**训练启动参数** ✓ 所以正在跑的训练不会变 —— 需重跑一次（可断点续训）✓
> ⚠️ 老项目里存着的值会按**新语义（轮）**理解 ✓ 例：之前填 10 → 现在就是每 10 轮 ✓
