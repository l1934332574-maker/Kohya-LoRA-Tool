...
# KohyaLoRA AMD distributed compatibility patch v1
#
    ...
    try:
        ...
        try:
            _probe = (
                ...
                "print('KOHYA_DIST_OK' if hasattr(d, 'is_initialized') else 'KOHYA_DIST_BAD')\n"
                "try:\n"
                " import torch.distributed.tensor\n"
                " print('KOHYA_DTENSOR_OK')\n"
                "except Exception:\n"
                " print('KOHYA_DTENSOR_BAD')\n"
            )
            r = subprocess.run([vpy, "-c", _probe],
                               capture_output=True, text=True, timeout=180)
            _probe_out = r.stdout or ""
            need = (r.returncode != 0 or "KOHYA_DIST_OK" not in _probe_out
                    or "KOHYA_DTENSOR_OK" not in _probe_out)
        except Exception:
            need = True
        if not need:
            return
        ...
        if os.path.isfile(target):
            try:
                ...
                # Never replace a sitecustomize we could not inspect: it may contain
                # user hooks that must be preserved.
                logf(f"[AMD] 无法读取现有 sitecustomize.py，跳过兼容补丁以避免覆盖用户内容：{e}")
                return False
        if "KohyaLoRA AMD distributed compatibility patch v1" in existing:
            return True
        with open(target, "a" if existing else "w", encoding="utf-8") as f:
            if existing and not existing.endswith("\n"):
                f.write("\n")
            if existing:
                f.write("\n")
            f.write(_AMD_DIST_PATCH)
        logf("[AMD] 检测到 torch.distributed 残缺，已追加兼容补丁（保留现有 sitecustomize.py）")
        return True
    except Exception as e:
...
def _file_signature(path):
    """Return a cheap identity for a file, including same-size rewrites."""
    try:
        st = os.stat(path)
...
def _valid_safetensors_file(path):
    """Validate a safetensors header and declared payload bounds without loading weights."""
    try:
        ...
        with open(path, "rb") as f:
            ...
            if header_size < 2 or header_size > 16 * 1024 * 1024 or 8 + header_size > size:
                return False
        ...
        for name, desc in header.items():
            ...
            if not isinstance(desc, dict):
                return False
            if (not isinstance(desc.get("dtype"), str)
                    or not isinstance(desc.get("shape"), list)
                    or not all(isinstance(x, int) and x >= 0 for x in desc["shape"])):
                return False
            ...
            tensors += 1
        return tensors > 0 and size == 8 + header_size + max_offset
    except Exception:
        return False


def _amd_run_saved_complete_model(log_tail, model_path, model_before):
    """Recognize an AMD end-of-training crash only with 100%, loss, and a new valid LoRA."""
    try:
        blob = "\n".join(str(line) for line in (log_tail or ()))
        complete = False
        for line in blob.splitlines():
            if not re.search(r"\bsteps:\s*100%", line, re.I):
                continue
        ...
        loss_match = re.search(
            r"\bavr_loss\s*=\s*([-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?)",
            blob, re.I)
        if not complete or not loss_match:
            return False
        loss = float(loss_match.group(1))
        if not (-float("inf") < loss < float("inf")):
            return False
        current = _file_signature(model_path)
        return current is not None and current != model_before and _valid_safetensors_file(model_path)
    except Exception:
        return False
...
def _require_healthy_amd_venv(venv_dir):
    """拒绝把 pip 操作写进跨用户迁移后已失效的 AMD venv。"""
    vpy = os.path.join(venv_dir, "Scripts", "python.exe")
    if not os.path.isfile(vpy):
        raise RuntimeError(
            "AMD 训练环境无效，找不到 Python：%s\n"
            "请在 AMD 环境引导里点「自动创建训练环境」，再安装依赖。" % vpy)
    ok, detail = _venv_python_ok(vpy)
    if not ok:
        raise RuntimeError(
            "AMD 训练环境已损坏（%s）。\n"
            "请在 AMD 环境引导里点「自动创建训练环境」重建；旧环境会改名保留。" % detail)
    return vpy
...
def install_amd_rocm(venv_dir, logf=print, progress_cb=None, status_cb=None):
    ...
    """
    _require_healthy_amd_venv(venv_dir)
...
def verify_amd_torch(venv_dir):
    ...
    _vok, _vdetail = _venv_python_ok(py)
    if not _vok:
        return False, "AMD 训练环境已损坏（%s），请重新创建环境。" % _vdetail, False
    code = (
        "import torch;"
...
def create_python_venv(py_ver, target, logf=print, py_exe=None):
    ...
    try:
        ...
        vpy = os.path.join(target, "Scripts", "python.exe")
        if os.path.isfile(vpy):
            ok, detail = _venv_python_ok(vpy)
            if ok and _ensure_venv_pip(vpy, target, logf, label="AMD"):
                return True, "环境已存在，检查通过"
            logf("[AMD] ⚠ 现有训练环境不可用（%s），将保留旧环境并重建…" %
                 (detail if not ok else "pip 自愈失败"))
        elif os.path.isdir(target) and os.listdir(target):
            logf("[AMD] ⚠ 发现未完成的训练环境，将保留旧目录并重建…")

        py_exe = py_exe or custom_python_exe()
        if not py_exe and not py_ver:
            return False, "无法确定用于重建环境的 Python 版本；请安装 Python 3.12 后点「重新检查」。"
        if os.path.isdir(target):
            stamp = time.strftime("%Y%m%d_%H%M%S")
            backup = target + "_broken_" + stamp
            suffix = 1
            while os.path.exists(backup):
                backup = target + "_broken_" + stamp + "_%d" % suffix
                suffix += 1
            try:
                os.rename(target, backup)
                logf("[AMD] 旧训练环境已保留到：%s" % backup)
            except Exception as e:
                return False, "旧训练环境无法改名保留（%s），请先关闭占用它的程序后重试。" % e
        ...
        if r.returncode == 0 and os.path.isfile(vpy):
            ok, detail = _venv_python_ok(vpy)
            if not ok:
                return False, "新建训练环境仍无法启动（%s）。请检查 Python 安装后重试。" % detail
            if not _ensure_venv_pip(vpy, target, logf, label="AMD"):
                return False, "新建训练环境缺少 pip，自动修复失败；请修复 Python 安装后重试。"
...
def _ensure_tokenizer_cached(cache_dir, model_id, logf=print, kind="clip", vpy=None):
    ...
    def _complete_dir(d):
        try:
            ...
            # auto：fast tokenizer 用 tokenizer.json；sentencepiece 用 spiece.model；
            # Qwen3 slow tokenizer 可由 vocab.json + merges.txt 加载。
            return (_ok("tokenizer_config.json")
                    and (_ok("tokenizer.json") or _ok("spiece.model")
                         or (_ok("vocab.json") and _ok("merges.txt"))))
        except Exception:
            return False
    ...
    if kind == "clip":
        ...
        _cands = ["tokenizer_config.json", "special_tokens_map.json", "tokenizer.json",
...
def _guard_anima_base_model(base_type, base_model):
    """阻止把 Anima 的 Qwen3-0.6B 文本编码器误当作 DiT 底模。"""
    if str(base_type or "").strip().lower() != "anima" or not base_model:
        return
    candidates = [os.path.basename(os.path.normpath(str(base_model)))]
    if os.path.isfile(str(base_model)):
        candidates.append(os.path.basename(os.path.dirname(str(base_model))))
    for candidate in candidates:
        compact = re.sub(r"[^a-z0-9]", "", candidate.lower())
        if "qwen306b" in compact:
            raise RuntimeError(
                "所选文件看起来是 Qwen3-0.6B 文本编码器，不是 Anima 底模。\n\n"
                "Anima 底模请选择 anima-base-v1.0.safetensors；Qwen3-0.6B 是配套文本编码器，"
                "请在 Anima 组件设置中指定，或让程序自动查找/下载。")
    if os.path.isdir(str(base_model)):
        try:
            model_type, architectures = _read_cfg_identity(os.path.join(str(base_model), "config.json"))
            identity = " ".join([model_type] + architectures).lower()
            if "qwen3" in identity or ("qwen" in identity and "causallm" in identity):
                raise RuntimeError(
                    "所选目录是 Qwen3 文本编码器，不是 Anima DiT 底模。\n\n"
                    "Anima 底模请选择 anima-base-v1.0.safetensors；Qwen3-0.6B 是配套文本编码器，"
                    "请在 Anima 组件设置中指定，或让程序自动查找/下载。")
        except RuntimeError:
            raise
...
def train(logf=print, base_model=None, mode="style", params=None, vram_gb=None, resume_from=None, progress=None):
    params = params or {}
    _guard_anima_base_model(params.get("base_type", "sd15"), base_model or params.get("base_model"))
    ...
    if amd_mode:
        ...
        _venv_ok, _venv_detail = _venv_python_ok(vpy)
        if not _venv_ok:
            raise RuntimeError(
                "AMD 训练环境已损坏（%s）。\n\n"
                "打开「环境检查 / 安装引导」，确认「训练环境」路径后点「自动创建训练环境」；"
                "旧环境会改名保留。" % _venv_detail)
        _bk = detect_torch_backend(vpy)
    ...
    data_sub("output")
    data_sub("logs")
    for _tid, _kind in _training_tokenizers(arch_info, family):
    ...
    if rc != 0 and _log_mentions_tokenizer_failure(_log_tail):
        ...
        if family == "anima":
            _ensure_anima_qwen3_tokenizer(qwen3, logf, force=True)
        _log_tail.clear()
        rc = run_stream(cmd, cwd=sds, env=env, logf=logf, collect=_log_tail)
    ...
    if rc != 0 and amd_mode and _amd_run_saved_complete_model(_log_tail, model_path, model_before):
        logf("[AMD] 训练步数已完成 100%，loss 已记录，且检测到本次新写入的有效 LoRA 文件；"
             "将本次训练标记为完成。子进程退出码 %s 属于训练收尾阶段异常，可在日志中继续查看原因。" % rc)
        rc = 0
    if rc != 0:
        if progress is not None:
            try:
...
_ANIMA_QWEN3_TOKENIZER_FILES = (
    "tokenizer_config.json", "special_tokens_map.json", "tokenizer.json",
    "vocab.json", "merges.txt",
)


def _anima_qwen3_tokenizer_complete(path):
    """Anima 的自定义训练脚本从 --qwen3 目录本身读取 tokenizer。"""
    ...
    def _file_ok(name):
        ...
        if name.endswith(".json"):
            try:
                ...
                return False
        return True
    return (_file_ok("tokenizer_config.json") and
            (_file_ok("tokenizer.json") or
             (_file_ok("vocab.json") and _file_ok("merges.txt"))))


def _ensure_anima_qwen3_tokenizer(qwen3_path, logf=print, force=False):
    """把 tokenizer 修到 Anima 实际传给训练器的 Qwen3 模型目录。"""
    if not qwen3_path or not os.path.isdir(qwen3_path):
        # 单 safetensors 模式由 sd-scripts 自带 configs/qwen3_06b 提供 tokenizer。
        return True
    if _anima_qwen3_tokenizer_complete(qwen3_path) and not force:
        return True

    stage = data_sub("cache", "anima_qwen3_tokenizer_repair")
    os.makedirs(stage, exist_ok=True)
    if force:
        for name in _ANIMA_QWEN3_TOKENIZER_FILES:
            try:
                os.remove(os.path.join(stage, name))
            except OSError:
                pass
    else:
        # 迁移旧的平铺 tokenizer 缓存到训练器实际读取的位置；只复制校验通过的文件。
        cache = os.path.join(data_sub("tokenizers"), "Qwen_Qwen3-0.6B")
        if os.path.isdir(cache):
            for name in _ANIMA_QWEN3_TOKENIZER_FILES:
                ...
                try:
                    ...
                    pass
        # 已有模型目录里的有效 tokenizer 资产也可复用。
        for name in _ANIMA_QWEN3_TOKENIZER_FILES:
            src = os.path.join(qwen3_path, name)
            if not os.path.isfile(src) or os.path.getsize(src) <= 0:
                continue
            if name.endswith(".json"):
                try:
    ...
    def _download(name):
        ...
        sources = (
            "https://modelscope.cn/models/Qwen/Qwen3-0.6B/resolve/master/" + name,
            "https://hf-mirror.com/Qwen/Qwen3-0.6B/resolve/main/" + name,
        )
        for url in sources:
            try:
                if _download_with_resume(url, dest, logf, direct=True) and _stage_ok(name):
                    logf("[Anima] tokenizer 组件已缓存：%s" % name)
                    return True
            except Exception as e:
                logf("[Anima] tokenizer 下载源失败（%s）：%s" % (name, e))
        return False

    if not _stage_ok("tokenizer_config.json") and not _download("tokenizer_config.json"):
        raise RuntimeError(
            "Qwen3 tokenizer 修复失败：无法取得 tokenizer_config.json。\n"
            "Anima 训练会直接从文本编码器目录加载 tokenizer，请检查网络或目录写入权限。")
    if not _stage_ok("tokenizer.json"):
        _download("tokenizer.json")
    if not _stage_ok("tokenizer.json"):
        for name in ("vocab.json", "merges.txt"):
            if not _stage_ok(name) and not _download(name):
                raise RuntimeError(
                    "Qwen3 tokenizer 修复失败：缺少 tokenizer.json，且无法取得 vocab.json / merges.txt。\n"
                    "训练器读取的目录：%s\n请检查网络和该目录的写入权限。" % qwen3_path)
    if not (_stage_ok("tokenizer.json") or
            (_stage_ok("vocab.json") and _stage_ok("merges.txt"))):
        raise RuntimeError(
            "Qwen3 tokenizer 仍不完整：需要 tokenizer.json，或同时具备 vocab.json 与 merges.txt。\n"
            "训练器读取的目录：%s" % qwen3_path)
    if not _stage_ok("special_tokens_map.json"):
        _download("special_tokens_map.json")  # 该文件可选，不阻止 AutoTokenizer

    logf("[Anima] tokenizer 将同步到训练器实际读取的文本编码器目录：%s" % qwen3_path)
    for name in _ANIMA_QWEN3_TOKENIZER_FILES:
        src = os.path.join(stage, name)
        if os.path.isfile(src):
            try:
                shutil.copy2(src, os.path.join(qwen3_path, name))
            except Exception as e:
                raise RuntimeError(
                    "无法将 Qwen3 tokenizer 写入训练器读取目录：%s\n%s\n"
                    "请检查目录写入权限，或选择可写的 Qwen3 模型目录。" % (qwen3_path, e))
    if not _anima_qwen3_tokenizer_complete(qwen3_path):
        raise RuntimeError("Qwen3 tokenizer 同步后仍不完整：%s" % qwen3_path)
    try:
...
def _training_tokenizers(arch_info, family):
    """Anima tokenizer 由 --qwen3 目录提供，避免写到训练器不会读取的通用缓存。"""
    specs = list((arch_info or {}).get("tokenizers") or [])
    if family == "anima":
        specs = [(model_id, kind) for model_id, kind in specs
...
def _ensure_anima_components(logf=print):
    ...
    # 训练脚本的 AutoTokenizer.from_pretrained(qwen3_path, local_files_only=True)
    # 直接读取 --qwen3 指向的目录；通用 tokenizers 缓存目录并不是它的读取位置。
    _ensure_anima_qwen3_tokenizer(qwen3_path, logf)
