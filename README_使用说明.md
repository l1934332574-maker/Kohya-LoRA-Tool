...
> v0.6.4 起：AMD 版 PyTorch 的 torch.distributed 若为残缺构建（训练收尾崩、模型不保存），工具会自动写入条件生效的兼容层（sitecustomize），无需手动修补。
...
### 新手引导（数据驱动，v0.6.1 起按模式自动生成）
- **主页（项目列表）不显示引导**：先打开/新建项目，再选训练模式，左侧才会出现对应引导。
...
### AI Toolkit 图像 LoRA（Qwen-Image / Z-Image，实验性）

- 新增 **「🖼 Qwen-Image LoRA」** 与 **「🖼 Z-Image LoRA」** 两个模式，走第三引擎 AI Toolkit。
- **Qwen-Image（20B）**：可选 `Qwen/Qwen-Image-2512`（默认）或 `Qwen/Qwen-Image-2.1`，约 40GB。显存：**16G 起步、24G 舒服（推荐）**。
- **Z-Image（8B）**：模型 `Tongyi-MAI/Z-Image`，约 16GB。显存：**12G 起步、16G 舒服**。
- **Qwen-Image 训练步骤**
  1. 打开或新建项目，在左侧模式导航选择 **「🖼 Qwen-Image LoRA」**。如果顶部状态提示第三引擎未安装，点 **「⚙ 安装第三引擎」**；已安装则跳过。若顶部出现动态的 **「↑ 引擎更新可用」** 提示，点击后在弹窗中点 **「一键更新」**，完成后再开始训练。
  2. 点顶部 **「选择训练模型」**，从下拉框选 **Qwen-Image-2512（默认）** 或 **Qwen-Image-2.1**，再点 **「使用所选模型」**。工具会自动匹配训练架构，不用填写仓库名或架构。选择保存在本机的 Qwen-Image 模式设置里，其他项目也会沿用；开训前确认弹窗会再次显示实际模型仓库。两个版本分别缓存，各约占 40GB。
  3. 如果模型已经在本机：先在下拉框选相应版本。2512 和 Z-Image 选择完整的 AI Toolkit Diffusers 模型目录，需含 `model_index.json`、`transformer/config.json`、`text_encoder/config.json` 和对应权重文件。**Qwen-Image-2.1** 还可点 **「选择 Qwen-Image-2.1 权重文件…」**，直接选 ComfyUI 中的 `qwen_image_2.1_bf16.safetensors`（或同版本兼容权重）；不要选整个 `models\unet` 文件夹。工具会自动在同一 ComfyUI `models` 目录下的 `clip` / `text_encoders` 与 `vae` 中查找现有组件并直接加载；只在本机缺失时下载对应组件。Qwen 仓库仍会提供小型配置和 processor 文件。
  4. 选训练类型：**人物**训练同一角色，**画风**训练整体视觉风格，**概念**训练形态/服装/物品等概念。人物和概念建议填写专属 Trigger。选择原始图片文件夹，至少准备 **15 张**清晰、内容一致的图片；WD14 会自动生成初始标签，训练前可用「标签编辑器」检查和修正。
  5. 点左侧 **「🚀 一键开始训练」**。它先自动预处理图片（去重、过滤过小或模糊图片、按裁切设置处理、WD14 打标签），然后弹出处理结果和训练参数确认；确认后开始训练。只想手动检查流程时，可单独点「数据预处理」，但一键训练本身会自动预处理。默认会跳过项目数据集中已存在的同名图片；修改过标签后要勾选「重新处理已存在的图片」。
  6. 若没有指定本地模型且缓存未就绪，训练阶段会下载所选模型到程序数据目录下的 `models/at_image/`；Qwen-Image 模型约 **40GB**，默认尝试 ModelScope 国内直链并支持中断续传。若选了 Qwen-Image-2.1 单文件权重，工具会复用该权重，并自动使用本机 ComfyUI 中已有的文本编码器和 VAE。
  7. 训练完成后，LoRA 在当前项目的 `output/<项目名>/` 文件夹；用界面中的「打开输出文件夹」查看。
- **模型选择注意**：2512 与 2.1 共用 Qwen-Image 模式，但使用各自匹配的训练架构。切换模型后，训练确认弹窗里的模型仓库应与目标版本一致；若不一致，返回「选择训练模型」重新选择。
- 默认训练步数为 2000；参数确认窗口显示本次实际类型、模型、Trigger 和步数。
