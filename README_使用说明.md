...
- **Qwen-Image 训练步骤**
  ...
  3. 如果模型已经在本机：先在下拉框选相应版本，再点 **「选择已有模型目录…」**。选择完整的 AI Toolkit diffusers 模型目录，里面要有 `model_index.json`、`transformer/config.json`、`text_encoder/config.json` 和对应权重文件。单个 `.safetensors` 文件或 ComfyUI 的总模型目录不能代替这个目录；指定完成后训练不会再下载底模。
  ...
  6. 若没有指定本地目录且模型缓存未就绪，训练阶段会下载约 **40GB** 到程序数据目录下的 `models/at_image/`；默认尝试 ModelScope 国内直链并支持中断续传，直链不可用时会尝试训练时在线加载。需要预留模型存储空间和较长下载时间。
