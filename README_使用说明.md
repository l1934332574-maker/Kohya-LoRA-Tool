...
- **Qwen-Image 训练步骤**
  ...
  6. 若没有指定本地模型且缓存未就绪，训练阶段会下载所选模型到程序数据目录下的 `models/at_image/`；Qwen-Image 模型约 **40GB**，默认尝试 ModelScope 国内直链并支持中断续传。若选了 Qwen-Image-2.1 单文件权重，工具会复用该权重，但首次训练仍需联网加载其余组件。
